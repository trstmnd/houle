...
Le code fait foi sur GitHub : `trstmnd/houle`. Le Drive n'est qu'un clone.

| Événement | Ce qui se passe | URL |
|---|---|---|
| Push sur n'importe quelle branche | Actions : `check.sh`, puis déploiement du dossier `game/` sur `gh-pages` dans `preview/<branche>/` | `https://trstmnd.github.io/houle/preview/<branche>/` |
| Push sur `main` | Idem, à la racine | `https://trstmnd.github.io/houle/` |
| `check.sh` rouge | Rien n'est déployé, le run Actions est rouge | |
...
2. L'agent code un bloc, `sh check.sh`, commit, push, **donne l'URL de preview**.
3. Tristan ouvre l'URL, joue, dit une sensation. Jamais deux fonctionnalités sans tester entre les deux.
4. Fin de session : « merge ». L'agent ouvre la PR, Tristan la merge depuis l'app GitHub, `main` se déploie.
